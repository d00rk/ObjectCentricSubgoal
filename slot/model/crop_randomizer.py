...
from slot.utils.tensor_utils import join_dimensions, reshape_dimensions, unsqueeze_expand_at, flatten
...
class CropRandomizer(nn.Module):
    ...
    def forward_out(self, inputs):
        if self.num_crops <= 1:
            ...
            out = reshape_dimensions(inputs, begin_axis=0, end_axis=0, target_dims=(batch_size, self.num_crops))
    ...
    def __repr__(self):
        header = '{}'.format(str(self.__class__.name))
        msg = header + f"(input_shape={self.input_shape}, crop_size=[{self.crop_height}, {self.crop_width}], num_crops={self.num_crops})"
        return msg
