    ...
    for epoch in range(cfg.train.num_epochs):
        ...
        if epoch % cfg.train.vis_every == 0:
            ...
            with torch.no_grad():
                ...
                image, meta = b
                image = image[:cfg.train.vis_n].to(device, non_blocking=True)           # (B, C, H, W)
                image_hwc = nchw_to_nhwc(image)
                recon_combined, recons, masks, slots = model(image_hwc)
                recon_combined = recon_combined.permute(0, 3, 1, 2).contiguous()        # (B, C, H, W)
